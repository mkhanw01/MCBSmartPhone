...
public class Database
{
    ...
    public static final String LOANPAYMENTS_TABLE_NAME = "loanpayments";
    public static  final String LOANPAYMENTS_ID="_id";

    public static  final String LOANPAYMENTS_CUSTOMERNAME="customer_name";
    public static   final String LOANPAYMENTS_LOANAMOUNT="loan_amount";
    public static  final String LOANPAYMENTS_INTERESTRATE="interest_rate";
    public static final String LOANPAYMENTS_MONTHS="month";
    public static final String LOANPAYMENTS_MONTHLYPAYMENTS="month_pay";
    public static final String LOANPAYMENTS_TOTALYPAYMENTS="totaly_pay";
    ...
    boolean addLoanpayments(Context context1, String _Id,
                            String customer_name, String loan_amount, String interest_rate,
                            String month, String month_pay, String totaly_pay)

    {
        ...
        try {
            dbhelper1 = new DBHelper(context1);
            db1 = dbhelper1.getWritableDatabase();
            db1.beginTransaction();

            // execute insert command
            ContentValues values = new ContentValues();
            values.put(Database.LOANPAYMENTS_ID,_Id);
            values.put(Database.LOANPAYMENTS_CUSTOMERNAME, customer_name);
            values.put(Database.LOANPAYMENTS_LOANAMOUNT, loan_amount);
            values.put(Database.LOANPAYMENTS_INTERESTRATE, interest_rate);
            values.put(Database. LOANPAYMENTS_MONTHS, month);
            values.put(Database.LOANPAYMENTS_MONTHLYPAYMENTS, month_pay);
            values.put(Database.LOANPAYMENTS_TOTALYPAYMENTS, totaly_pay);


            long rowid = db1.insert(Database.LOANPAYMENTS_TABLE_NAME, null,
                    values);
            Log.d("Accounts", "Inserted into TRANSACTIONS " + rowid);
            if (rowid != -1) {
                // update Accounts Table
                boolean done = Database.updateAccountBalance(db1, _Id,
                        customer_name, Double.parseDouble(loan_amount),interest_rate);
